...
export default function CitacoesPage() {
  ...
  const { handleDrill: handleDrillByTitle, drillDownProps } = useArticleDrillDown(filtered, (data, title) => {
    const needle = title.toLowerCase();
    return data.filter((w) => (w.TI ?? "").toLowerCase().includes(needle));
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <ChartContainer ref={topRef} title="Top 20 Mais Citados">
        <BarChart
          data={topCited.slice(0, 20).map((r) => ({ name: r.title.slice(0, 50) + (r.title.length > 50 ? "…" : ""), citações: r.citations, fullTitle: r.title }))}
          xKey="name" bars={[{ key: "citações", label: "Citações" }]} layout="vertical" height={500}
          onBarClick={(e) => handleDrillByTitle(String(e.fullTitle ?? e.name))}
        />
      </ChartContainer>
      ...
      <Card>
        ...
        <CardContent>
          <DataTable columns={columns} data={topCited} onRowClick={(row) => handleDrillByTitle(row.title)} />
        </CardContent>
      </Card>
