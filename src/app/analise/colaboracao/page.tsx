...
import { coauthorshipNetwork, countryCollaborationNetwork, institutionCollaborationNetwork, keywordCooccurrenceNetwork, filterWorksByField, extractCountries } from "@/lib/data-processing";
...
export default function ColaboracaoPage() {
  ...
  const filterModeRef = useRef<"AU" | "C3" | "country" | "DE">("AU");
  const { handleDrill, drillDownProps } = useArticleDrillDown(filtered, (data: BibWork[], name: string) => {
    const mode = filterModeRef.current;
    if (mode === "country") {
      const indices = new Set(extractCountries(data).filter((e) => e.país === name).map((e) => e.index));
      return data.filter((_, i) => indices.has(i));
    }
    return filterWorksByField(data, mode, name);
  });
  const nodeHandler = useCallback((mode: "AU" | "C3" | "country" | "DE") =>
    (node: { id: string; label: string }) => {
      filterModeRef.current = mode;
      handleDrill(node.label);
    }, [handleDrill]);
      ...
      <Tabs defaultValue="coauthor">
        ...
        <TabsContent value="coauthor">
          <ChartContainer ref={coauthRef} title="Rede de Coautoria" actions={<ChartExportButton chartRef={coauthRef} fileName="rede-coautoria" />}>
            {coauthNet.nodes.length > 0 ? <NetworkGraph data={coauthNet} height={550} onNodeClick={nodeHandler("AU")} /> : <EmptyState />}
          </ChartContainer>
        </TabsContent>
        <TabsContent value="institution">
          <ChartContainer ref={instRef} title="Rede de Colaboração Institucional" actions={<ChartExportButton chartRef={instRef} fileName="rede-instituicoes" />}>
            {instNet.nodes.length > 0 ? <NetworkGraph data={instNet} height={550} onNodeClick={nodeHandler("C3")} /> : <EmptyState />}
          </ChartContainer>
        </TabsContent>
        <TabsContent value="country">
          <ChartContainer ref={countryRef} title="Rede de Colaboração entre Países" actions={<ChartExportButton chartRef={countryRef} fileName="rede-paises" />}>
            {countryNet.nodes.length > 0 ? <NetworkGraph data={countryNet} height={550} onNodeClick={nodeHandler("country")} /> : <EmptyState />}
          </ChartContainer>
        </TabsContent>
        <TabsContent value="keyword">
          <ChartContainer ref={kwRef} title="Rede de Coocorrência de Palavras-chave" actions={<ChartExportButton chartRef={kwRef} fileName="rede-keywords" />}>
            {kwNet.nodes.length > 0 ? <NetworkGraph data={kwNet} height={550} onNodeClick={nodeHandler("DE")} /> : <EmptyState />}
          </ChartContainer>
        </TabsContent>
      </Tabs>
