...
export default function HomePage() {
  ...
  const [oaParams, setOaParams] = useState<OpenAlexSearchParams>({
    ...
    searchScope: "title_and_abstract" as const,
    author: "",
    source: "",
    institution: "",
    doi: "",
  ...
  const handleCheckCount = useCallback(async () => {
    const hasTopicFilter = !!oaParams.topic || (oaParams.topicIds?.length ?? 0) > 0;
    if (!hasTopicFilter && !oaParams.author && !oaParams.source && !oaParams.institution && !oaParams.doi) {
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    <Label>Escopo da busca textual</Label>
                    <Select
                      value={oaParams.searchScope ?? "title_and_abstract"}
                      onValueChange={(v) => setOaParams((p) => ({ ...p, searchScope: v as "title_and_abstract" | "title" | "fulltext" }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="title_and_abstract">Título e Resumo</SelectItem>
                        <SelectItem value="title">Somente Título</SelectItem>
                        <SelectItem value="fulltext">Texto Completo</SelectItem>
                      </SelectContent>
                    </Select>
                    <p className="text-xs text-muted-foreground">
                      Define onde os termos são procurados. &quot;Título e Resumo&quot; é mais preciso.
                    </p>
                  </div>
                  <div className="space-y-1.5">
                    <Label>Autor</Label>
                  ...
                  <div className="flex items-center gap-2 text-sm p-3 rounded-md bg-muted">
                    ...
                    <span>
                      ...
                      {oaCount > (oaParams.maxRecords ?? 1000) && (
                        <span className="text-muted-foreground"> Serão obtidos os primeiros {oaParams.maxRecords?.toLocaleString("pt-BR")}.</span>
                      )}
                    </span>
                  </div>
                )}
          ...
          <TabsContent value="saved">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Acesse análises salvas anteriormente no navegador. Selecione uma para carregar.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <SavedAnalysesList />
