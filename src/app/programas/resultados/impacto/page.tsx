...
export default function ProgramasImpactoPage() {
  const { programs, isReady } = usePrograms();

  const citBoxPlots = useMemo(
    () => (isReady ? computeCitationBoxPlots(programs) : []),
    [programs, isReady],
  );

  const fwciBoxPlots = useMemo(
    () => (isReady ? computeFwciBoxPlots(programs) : []),
    [programs, isReady],
  );

  const topCited = useMemo(
    () => (isReady ? computeTopCitedComparison(programs) : []),
    [programs, isReady],
  );

  const percentiles = useMemo(
    () => (isReady ? computePercentileComparison(programs) : []),
    [programs, isReady],
  );
  ...
  if (!isReady) {
    return (
      <div>
        <PageHeader title="Impacto" badge="Programas" />
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Impacto" description="Distribuição de citações, FWCI, artigos mais citados e percentis." badge="Programas" />
        ...
        <TabsContent value="fwci" className="mt-4">
          <ChartContainer title="Distribuição de FWCI" description="Field-Weighted Citation Impact — disponível apenas para dados do OpenAlex.">
            {fwciBoxPlots.some((b) => b.max > 0) ? (
              ...
              <EmptyState message="Dados de FWCI indisponíveis para os programas selecionados." />
            )}
          </ChartContainer>
        </TabsContent>

        <TabsContent value="topcited" className="mt-4">
          <ChartContainer title="Top 10 Artigos Mais Citados" description="Artigos com maior número de citações em cada programa.">
