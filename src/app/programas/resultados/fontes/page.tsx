...
export default function ProgramasFontesPage() {
  ...
  const topSources = useMemo(
    () => (isReady ? computeTopSourcesComparison(programs, topN) : []),
  ...
  if (!isReady) {
    return (
      <div>
        <PageHeader title="Fontes" badge="Programas" />
        <EmptyState message="Carregando dados dos programas..." icon={<Newspaper className="size-10" />} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Fontes" description="Sobreposição de periódicos, ranking comparativo e zonas de Bradford." badge="Programas" />
      ...
      <Tabs defaultValue="overlap">
        ...
        <TabsContent value="overlap" className="mt-4">
          {overlap && (
            <ChartContainer title="Sobreposição de Fontes" description="Periódicos compartilhados e exclusivos entre os programas.">
              <OverlapDisplay result={overlap} datasets={programs} label="fontes" />
            </ChartContainer>
          )}
        </TabsContent>
        ...
        <TabsContent value="ranking" className="mt-4 space-y-4">
          ...
          <ChartContainer title={`Top ${topN} Fontes`} description="Periódicos com mais publicações em cada programa.">
            <ComparisonBarChart
              data={topSources}
              categoryKey="name"
              datasets={programs}
        ...
        <TabsContent value="bradford" className="mt-4">
          <ChartContainer title="Zonas de Bradford" description="Distribuição de periódicos pelas 3 zonas de Bradford.">
